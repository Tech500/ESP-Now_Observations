...
String processor1(const String& var)
{
  ...
  if (var == F("TEMP"))
    return String(incomingReadings.temp, 2);

  if (var == F("HEATINDEX"))
    return String(incomingReadings.heat, 2);

  if (var == F("HUM"))
    return String(incomingReadings.hum, 2);

  if (var == F("DEWPOINT"))
    return String(incomingReadings.dew, 2);

  if (var == F("PRESSURE"))
    return String(incomingReadings.press, 3);
